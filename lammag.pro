...
;Calculating the advective terms for B_r
    ...
    FOR j = 1, nj-1 DO BEGIN
       term1(nc) = float(l)/r(1)*cheb(nc,1)
       term2(nc) = 2.*od*dcheb(nc,1)
       term3(nc) = 2.*od*dcheb(nc,1)*cmb*float(l)/r(1)
       term4(nc) = cmb1*(4.*od*od*d2cheb(nc,1)-
   $      float(l*(l+1))/(r(1)*r(1))*cheb(nc,1))
       term5(nc) = cmb1*cmb*float(l)/(2.*r(1))*
   $      (4.*od*od*d2cheb(nc,1)-float(l*(l+1))/(r(1)*r(1))*
   $       cheb(nc,1))
