...
PRO urtheta2,nn=nni,ni=ni,nj=nj
  COMPILE_OPT IDL2
    ...
    FOR n=0,nn-1 DO BEGIN
        FOR k=0,ni-1 DO BEGIN
            ...
            FOR g=0,nj-1 DO BEGIN
                tempr=tempr+data[0,n,k,g]^2
                temptheta=temptheta+data[1,n,k,g]^2
                tempphi=tempphi+data[2,n,k,g]^2
            ENDFOR
           tempr=tempr/(nj)
           temptheta=temptheta/(nj)
           tempphi=tempphi/(nj)
           ur[n,k]=tempr
           utheta[n,k]=temptheta
           uphi[n,k]=tempphi
        ENDFOR
    ENDFOR
    meanur=meanur+ur
    meanutheta=meanutheta+utheta
    meanuphi=meanuphi+uphi
  ENDFOR
...
;       XTITLE='radius', YTITLE='colatitude', YRANGE=[ni-1,0]
  ...
  Polar_Contour2, transpose(meanutheta), colatitude, $
        ...
        XRANGE=[0,50], YRANGE=[-50,50], YSTYLE=1
;       XTITLE='radius', YTITLE='colatitude', YRANGE=[ni-1,0]
  polar_contour2, blanktheta, colatitude, radb, $
        /CELL_FILL, NLEVEL=1, /OVERPLOT
  COLORBAR, COLOR=0, DIVISIONS=4, FORMAT='(I6)', $
       POSITION=[0.60, 0.08, 0.64, 0.96], /VERTICAL, /RIGHT,  $
  ...
  Polar_Contour2, transpose(meanuphi), colatitude, $
        ...
        XRANGE=[0,50], YRANGE=[-50,50], YSTYLE=1
;        XTITLE='radius', YTITLE='colatitude', YRANGE=[ni-1,0]
  polar_contour2, blankphi, colatitude, radb, $
        /CELL_FILL, NLEVEL=1, /OVERPLOT
  COLORBAR, COLOR=0, DIVISIONS=4, FORMAT='(I6)', $
       POSITION=[0.60, 0.08, 0.64, 0.96], /VERTICAL, /RIGHT,  $
